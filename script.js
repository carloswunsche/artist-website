...
function updateLyricsPanel(track) {
  ...
  lrcLines = [];
  if (!has) { if (lyricsOpen) { lyricsOpen = false; toggleLyrics(); btn.classList.remove('active'); } return; }
  ...
  if (lyricsOpen) toggleLyrics(); // ensure open state consistent
}
...
function toggleLyrics() {
  ...
  document.getElementById('lyrics-btn').classList.toggle('active', lyricsOpen);
}
...
function setPP(p) {
  document.getElementById('ico-play').style.display = p ? 'block' : 'none';
  document.getElementById('ico-pause').style.display = p ? 'none' : 'block';
}
...
function renderArtist(d, base) {
  ...
  // Load first track into player (no autoplay)
